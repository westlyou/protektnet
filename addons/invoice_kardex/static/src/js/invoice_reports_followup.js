odoo.define('invoice_kardex.invoice_report_followup', function (require) {
'use strict';
...
var invoice_report = require('invoice_kardex.invoice_report');

var QWeb = core.qweb;

var invoice_report_followup = invoice_report.extend({
    events: _.defaults({
    ...
    }, invoice_report.prototype.events),
    ...
    display_done: function(e) {
        $(e.target).parents('.o_invoice_reports_body').find("div.o_invoice_reports_page").find(".alert.alert-info.alert-dismissible").remove();
        $(e.target).parents('.o_invoice_reports_body').find('#action-buttons').addClass('o_invoice_reports_followup_clicked');
    ...
    send_followup_email: function(e) {
        ...
        return this._rpc({
            ...
            .then(function (result) { // send the email server side
                self.display_done(e);
                $(e.target).parents("div.o_invoice_reports_page").prepend(QWeb.render("emailSent")); // If all went well, notify the user
            });
    },
...
core.action_registry.add("invoice_report_followup", invoice_report_followup);
return invoice_report_followup;
});
