odoo.define('invoice_kardex.invoice_report', function (require) {
'use strict';
...
var InvoicekardexReportsWidget = Widget.extend(ControlPanelMixin, {

    events: {
        'click .js_invoice_report_foldable': 'fold_unfold',
    ...
    init: function(parent, action) {
        ...
        if (this.report_model === undefined) {
            this.report_model = 'invoice.kardex';
        }
        this.invoice_kardex_id = false;
        if (action.context.id) {
            this.invoice_kardex_id = action.context.id;
        }
        ...
        if ((action.ignore_session === 'read' || action.ignore_session === 'both') !== true) {
            var persist_key = 'report:'+this.report_model+':'+this.invoice_kardex_id+':'+session.company_id;
    ...
    start: function() {
        ...
        var extra_info = this._rpc({
                ...
                args: [self.invoice_kardex_id, self.report_options],
    ...
    render_searchview_buttons: function() {
        ...
        var $datetimepickers = this.$searchview_buttons.find('.js_invoice_reports_datetimepicker');
        ...
        // click event
        this.$searchview_buttons.find('.js_invoice_report_date_filter').click(function (event) {
    ...
    render_footnotes: function() {
        ...
        var $dom_footnotes = self.$el.find('.js_invoice_report_line_footnote:not(.folded)');
        $dom_footnotes.html('');
        var number = 1;
        ...
        _.each($dom_footnotes, function(el) {
            ...
            if (footnote.length !== 0) {
                $(el).html('<sup><b class="o_invoice_reports_footnote_sup"><a href="#footnote'+number+'">'+number+'</a></b></sup>');
        ...
        return this._rpc({
                ...
                args: [self.invoice_kardex_id, footnote_to_render],
                context: self.odoo_context,
            })
            .then(function(result){
                return self.$el.find('.js_invoice_report_footnotes').html(result);
            });
    },
    fold_unfold: function(e) {
        var self = this;
        if ($(e.target).hasClass('caret') || $(e.target).parents('.o_invoice_reports_footnote_sup').length > 0){return;}
        e.stopPropagation();
        e.preventDefault();
    ...
    fold: function(line) {
        ...
        if ($lines_to_hide.length > 0) {
            ...
            $lines_to_hide.find('.js_invoice_report_line_footnote').addClass('folded');
            $lines_to_hide.hide();
    ...
    unfold: function(line) {
        ...
        if ($lines_in_dom.length > 0) {
            $lines_in_dom.find('.js_invoice_report_line_footnote').removeClass('folded');
            $lines_in_dom.show();
            ...
            return this._rpc({
                    model: this.report_model,
                    method: 'get_html',
                    args: [self.invoice_kardex_id, self.report_options, line.data('id')],
                    context: self.odoo_context,
                })
                .then(function(result){
    ...
    unfold_all: function(bool) {
        var self = this;
        var lines = this.$el.find('.js_invoice_report_foldable');
    ...
    trigger_action: function(e) {
        ...
        if (action) {
            return this._rpc({
                    model: this.report_model,
                    method: action,
                    args: [this.invoice_kardex_id, this.report_options, params],
...
ActionManager.include({
    ir_actions_invoice_report_download: function(action, options) {
        var self = this;
        var c = crash_manager;
...
core.action_registry.add('invoice_report', InvoicekardexReportsWidget);
return InvoicekardexReportsWidget;
});
