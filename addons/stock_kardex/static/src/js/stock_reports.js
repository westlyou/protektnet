...
var stockardexReportsWidget = Widget.extend(ControlPanelMixin, {
    ...
    render_searchview_buttons: function() {
        ...
        // filter brands
        this.$searchview_buttons.find('.js_stock_reports_brand_auto_complete').select2();
        if (self.report_options.brand) {
            self.$searchview_buttons.find('[data-filter="brand_ids"]').select2("val", self.report_options.brand_ids);
        }
        this.$searchview_buttons.find('.js_stock_reports_brand_auto_complete').on('change', function(){
