...
class StockMove(models.Model):
    ...
    def _action_confirm(self):
        """
            Confirms stock move or put it in waiting if it's
            linked to another move.
        """
    ...
    def _action_done(self):
        """
            Process completly the moves given as ids and if all moves are
        ...
        check = False
        for obj in self:
            if (obj.location_id.usage == "inventory" or
                    obj.location_dest_id.usage == "inventory"):
                check = True
    ...
    def fetch_stock_warehouse(self):
        ctx = dict(self._context or {})
        productQuantity = 0
        connectionObj = self.env['magento.configure'].search(
            [('active', '=', True)], limit=1)
            ...
            for data in self:
                ...
                if data.origin:
                    ...
                    if saleObjs:
                        ...
                        if (get_channel == 'magento' and data.picking_id and
                                data.picking_id.picking_type_code ==
                                'outgoing'):
                            flag = 0
                else:
                    flag = 2  # no origin
    ...
    @api.model
    def check_warehouse(self, odooProductId, sku,
                        warehouseId, productQuantity):
        ctx = dict(self._context or {})
        ...
        if mappingObj:
            ...
            if mappingObj.instance_id.warehouse_id.id == warehouseId:
                ...
                if ctx.get('mob_stock_action_val') == "qoh":
                    productQuantity = (
                        productObj.qty_available - productObj.outgoing_qty)
                else:
                    productQuantity = productObj.virtual_available
                self.synch_quantity(
                    mageProductId, productQuantity, sku,
                    stockItemId, instanceObj)
        return True

    @api.model
    def synch_quantity(self, mageProductId, productQuantity,
                       sku, stockItemId, instanceObj):
    ...
    @api.model
    def update_quantity(self, mageProductId, productQuantity,
                        sku, stockItemId, instanceObj):
        text = ''
        ctx = dict(self._context or {})
            ...
            if not instanceObj.active:
                ...
                try:
                    ...
                    apiResponse = self.env[
                        ...
                        url='/V1/products/' + str(sku) + '/stockItems/' +
                            str(stockItemId),
                        method='put',
                        data=stockData,
                    )
                    ...
                    return [0, ' Error in Updating Quantity for Magneto '
                            'Product Id %s, Reason >>%s' % (
                                mageProductId, str(e))]
        else:
            return [
                0, 'Error in Updating Stock, Magento Product Id Not Found!!!']
